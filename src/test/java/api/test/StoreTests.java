...
public class StoreTests {
	...
	{
		logger.info("############# Store Inverntory launched #############");
		Response response=UserEndpoints.getStoreInventory();
		response.then()
		...
		logger.info("############# Store Inverntory end  #############");
	}
	
	@Test(priority=2)
	public void testStorePlaceOrder(){
		logger.info("############# Place Store Order #############");
		Response response=UserEndpoints.placeOrder(order);
		response.then()
			.log().all();
		Assert.assertEquals(response.getStatusCode(),200);
	...
	public void testStoreGetOrder()
	{
		...
		Response response=UserEndpoints.getOrder(this.order.getId());
		response.then()
			.statusCode(200)
			.log().all();
	...
	public void testStoreDeleteOrder()
	{
		...
		Response response=UserEndpoints.deleteOrder(this.order.getId());
		response.then()
			.statusCode(200)
			.log().all();
